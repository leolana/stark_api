...
import { checkDocumentExistenceStatusEnum as statusDocumento } from './checkDocumentExistenceStatusEnum';
import participateNominationSourceEnum from '../../entities/participateNominationSourceEnum';

const checkDocumentExistenceUseCase = db => (documento) => {
  const result = [];
  ...
  function checkCredenciamentos() {
    return db.entities.credenciamento.findAll({
      ...
      const mapping = {
        [credenciamentoStatusEnum.aprovado]: statusDocumento.credenciamentoAprovado,
        [credenciamentoStatusEnum.emAnalise]: statusDocumento.credenciamentoEmAnalise,
        [credenciamentoStatusEnum.pendente]: statusDocumento.credenciamentoPendente,
        [credenciamentoStatusEnum.reprovado]: statusDocumento.credenciamentoRecusado,
      };
  ...
  function checkIndicacoes() {
    return db.entities.participanteIndicacao.findAll({
      ...
      const mapping = {
        [participateNominationSourceEnum.indicacaoPorEc]: statusDocumento.indicadoPorEstabelecimento,
        [participateNominationSourceEnum.indicacaoPorFornecedor]: statusDocumento.indicadoPorFornecedor,
      };
...
export default checkDocumentExistenceUseCase;
